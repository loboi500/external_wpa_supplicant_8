			    ...
			    bool debug_print)
{
...
#ifdef CONFIG_WAPI_SUPPORT
	bool wapi = false;
	struct wapi_context *wapi_ctx = wpa_s->wapi;
#endif
	...
	wapi = ie && ie[1];
	if (wapi && ie && wapi_ctx) {
		wapi_ctx->bss_wapi_ie_len = 2 + ie[1];
		os_memcpy(wapi_ctx->bss_wapi_ie, ie, wapi_ctx->bss_wapi_ie_len);
	}
#endif
	...
	if (!wapi && ssid->proto == WPA_PROTO_WAPI) {
		if (debug_print)
			wpa_dbg(wpa_s, MSG_DEBUG,
				"   skip - non-WAPI network not allowed");
		return false;
	}
#endif
	if (!osen && !wpa &&
				       ...
				       union wpa_event_data *data)
{
	 ...
	 */
	if (wpa_s->wpa_proto == WPA_PROTO_WAPI && wpa_s->wapi) {
