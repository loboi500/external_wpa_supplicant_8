...
// Allow p2p 6G connection. Note that enable p2p 6G may cause some IOT issues.
// It's suggested to add setting or mechanism to make sure if peer does support
// 6G before making a p2p 6G connection.
const bool allow_6ghz = true;
    ...
    const std::string& passphrase)
{
	...
	if (wpas_p2p_group_add_persistent(
		...
		CHANWIDTH_USE_HT, he, 0, NULL, 0, 0, allow_6ghz)) {
		ret = -1;
	}
...
SupplicantStatus P2pIface::findInternal(uint32_t timeout_in_sec)
{
	...
	if (wpas_p2p_find(
		...
		nullptr, search_delay, 0, nullptr, 0, false)) {
		return {SupplicantStatusCode::FAILURE_UNKNOWN, ""};
	}
	return {SupplicantStatusCode::SUCCESS, ""};
}
    ...
    bool persistent, uint32_t go_intent)
{
	...
	int new_pin = wpas_p2p_connect(
	    ...
	    vht, CHANWIDTH_USE_HT, he, 0, nullptr, 0, allow_6ghz);
	if (new_pin < 0) {
		return {{SupplicantStatusCode::FAILURE_UNKNOWN, ""}, {}};
	}
	std::string pin_ret;
    ...
    bool persistent, SupplicantNetworkId persistent_network_id)
{
	...
	if (ssid == NULL) {
		if (wpas_p2p_group_add(
			...
			CHANWIDTH_USE_HT, he, 0, allow_6ghz)) {
			return {SupplicantStatusCode::FAILURE_UNKNOWN, ""};
		} else {
			return {SupplicantStatusCode::SUCCESS, ""};
		}
    ...
    const std::array<uint8_t, 6>& peer_address)
{
	...
	if (wpas_p2p_invite_group(
		...
		go_device_address.data(), allow_6ghz)) {
		return {SupplicantStatusCode::FAILURE_UNKNOWN, ""};
	}
	return {SupplicantStatusCode::SUCCESS, ""};
}
    ...
    const std::array<uint8_t, 6>& peer_address)
{
	...
	if (wpas_p2p_invite(
		...
		CHANWIDTH_USE_HT, 0, he, 0, allow_6ghz)) {
		return {SupplicantStatusCode::FAILURE_UNKNOWN, ""};
	}
	return {SupplicantStatusCode::SUCCESS, ""};
}
